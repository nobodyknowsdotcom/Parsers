...
for link in links:
    try:
        ...
        denied_list.append(link)
        errors +=1
        print('Exception caught on %s'%link)
        continue

print('Checking denied list (%s elements)'%len(denied_list))
for e in denied_list:
    try:
        category, subcategory, name, url, old_price, discount_price, aviable_sizes, image, description, about_model, shiping = parse_page(driver, url=link, sleep_interval=REQUEST_INTERVAL*2)
...
with open('items.csv', 'w', encoding='utf_8') as f:
    write = csv.writer(f)
